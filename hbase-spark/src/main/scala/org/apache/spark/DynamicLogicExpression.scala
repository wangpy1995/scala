...
import org.apache.spark.datasources.JavaBytesEncoder.JavaBytesEncoder
import org.apache.spark.datasources.{BytesEncoder, JavaBytesEncoder}
...
trait DynamicLogicExpression {
  def execute(columnToCurrentRowValueMap: java.util.HashMap[String, ByteArrayComparable],
...
trait CompareTrait {
  ...
  def execute(columnToCurrentRowValueMap:
              java.util.HashMap[String, ByteArrayComparable],
              valueFromQueryValueArray: Array[Array[Byte]]): Boolean = {
